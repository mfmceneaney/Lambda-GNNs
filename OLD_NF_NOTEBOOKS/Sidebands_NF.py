...
parser.add_argument('--switch_mask', action=argparse.BooleanOptionalAction,help='Tells model if it should alternate the mask order each layer (to give each index equal times being transformed)')
parser.add_argument('--sidebands', action=argparse.BooleanOptionalAction,help='if true, the model will only train on the sidebands of the mass spectrum')
parser.add_argument('--double', action=argparse.BooleanOptionalAction,help='if true, the model will double the input dimension')
parser.add_argument('--alternate', action=argparse.BooleanOptionalAction,help='if true, the model will alternate bitmask')
parser.add_argument('--balanced', action=argparse.BooleanOptionalAction,help='if true, uses balanced sig/bg ratio for both MC and data')
...
# print(f"sidebands: {sidebands}; double: {double}; latent_dim: {latent_dim}")
...
MC_balanced = "mc__data_balanced_dataset_3_25_23"
...
if(balanced):
    MCgraph_dataset = GraphDataset(prefix+MC_balanced)
else:
...
num_samples = 100
training_data_DATA = create_latent_data(DATAdataset, extractor,num_samples = num_samples, max_events = DATA_max_events, double = double,sidebands = False, given_dataset = True, in_dataset = DATAgraph_dataset)
training_data_MC = create_latent_data(MCdataset, extractor,num_samples = num_samples, max_events = MC_max_events, double = double,sidebands = False, given_dataset = True, in_dataset = MCgraph_dataset)

testing_data_DATA = create_latent_data(DATAdataset, extractor, mode = "test",num_samples = num_samples, max_events = DATA_max_events, double = double,sidebands =False, given_dataset = True, in_dataset = DATAgraph_dataset)
testing_data_MC = create_latent_data(MCdataset, extractor, mode = "test",num_samples = num_samples, max_events = MC_max_events, double = double,sidebands = False, given_dataset = True, in_dataset = MCgraph_dataset)

val_data_DATA = create_latent_data(DATAdataset, extractor, mode = "val",num_samples = num_samples, max_events = DATA_max_events, double = double,sidebands = False, given_dataset = True, in_dataset = DATAgraph_dataset)
val_data_MC = create_latent_data(MCdataset, extractor, mode = "val",num_samples = num_samples, max_events = MC_max_events, double = double,sidebands = False, given_dataset = True, in_dataset = MCgraph_dataset)

sb_training_data_DATA = create_latent_data(DATAdataset, extractor,num_samples = num_samples, max_events = DATA_max_events, sidebands = sidebands, given_dataset = True, in_dataset = DATAgraph_dataset, double = double)
sb_training_data_MC = create_latent_data(MCdataset, extractor,num_samples = num_samples, max_events = MC_max_events, sidebands = sidebands, given_dataset = True, in_dataset = MCgraph_dataset, double = double)

sb_testing_data_DATA = create_latent_data(DATAdataset, extractor, mode = "test",num_samples = num_samples, max_events = DATA_max_events, sidebands = sidebands, given_dataset = True, in_dataset = DATAgraph_dataset, double = double)
sb_testing_data_MC = create_latent_data(MCdataset, extractor, mode = "test",num_samples = num_samples, max_events = MC_max_events, sidebands = sidebands, given_dataset = True, in_dataset = MCgraph_dataset, double = double)

sb_val_data_DATA = create_latent_data(DATAdataset, extractor, mode = "val",num_samples = num_samples, max_events = DATA_max_events, sidebands = sidebands, given_dataset = True, in_dataset = DATAgraph_dataset, double = double)
sb_val_data_MC = create_latent_data(MCdataset, extractor, mode = "val",num_samples = num_samples, max_events = MC_max_events, sidebands = sidebands, given_dataset = True, in_dataset = MCgraph_dataset, double = double)
'''
...
masked_affine_flows_train_MC = get_masked_affine(latent_dim = latent_dim,num_layers = num_layers,alternate_mask = (not double), switch_mask = switch_mask)
...
masked_affine_flows_train_DATA = get_masked_affine(latent_dim = latent_dim,num_layers = num_layers,alternate_mask = (not double), switch_mask = switch_mask)
...
# masked_affine_model_DATA.load(model_DATA_path)
...
for i in range(len(plot_fpd)):
    for j in range(71):
        if((np.isnan(plot_fpd[i,j])) or (not math.isfinite(plot_fpd[i,j]))):
            plot_fpd[i,0] = 99999
