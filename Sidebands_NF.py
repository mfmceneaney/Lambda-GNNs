...
loss_MC_path = loss_dir + "MCloss.jpeg"
loss_DATA_path = loss_dir + "DATAloss.jpeg"

model_dir = lambda_prefix + "/models/NF_sidebands_double/" + Date
model_MC_path = model_dir +"MC.pth"
model_DATA_path = model_dir +"DATA.pth"

smearing_dir = lambda_prefix + "/plots/NF_sidebands/smear/" + Date
smearing_path = smearing_dir + "sideband_double_smear.jpeg"

histo_dir = lambda_prefix + "/plots/NF_sidebands/histos/" + Date
histo_path = histo_dir + "mass_spectrum.jpeg"

FOM_pure_dir = lambda_prefix + "/plots/NF_sidebands/FOM_pure/" + Date
FOM_pure_path = FOM_pure_dir + "FOM_pure_plot.jpeg"

dirs = [loss_dir,model_dir,smearing_dir,histo_dir]
...
# masked_affine_flows_train_DATA = get_masked_affine(latent_dim = 142,num_layers = num_layers,alternate_mask = False,hidden_dim = 142 * 2)
# distribution_DATA = nf.distributions.DiagGaussian(142, trainable = False)
...
# masked_affine_flows_train_MC = get_masked_affine(latent_dim = 142,num_layers = num_layers,alternate_mask = False, hidden_dim = 142 * 2)
# distribution_MC = nf.distributions.DiagGaussian(142, trainable = False)
...
# # TRAINING MC
# loss_hist_MC, val_hist_MC, full_loss_hist_MC, full_val_hist_MC = train(sb_training_MC, MC_model, val = True, val_data = sb_val_MC, num_epochs = num_epochs_MC, compact_num = 20)
...
# # TRAINING DATA
# loss_hist_DATA, val_hist_DATA, full_loss_hist_DATA, full_val_hist_DATA = train(sb_training_DATA, DATA_model, val = True, val_data = sb_val_DATA, num_epochs = num_epochs_DATA, compact_num = 20)
...
distribution_MC = nf.distributions.DiagGaussian(142, trainable = False)
masked_affine_model_MC = nf.NormalizingFlow(q0=distribution_MC, flows=masked_affine_flows_train_MC)
masked_affine_model_MC.load(model_MC_path)
MC_model = masked_affine_model_MC.to(device)
...
probs_DATA_test = test_classifier_data(full_pass_DATA_obj, classifier,ret_probs = True)
probs_DATA_fp = test_classifier_data(full_pass_train_DATA_obj, classifier,ret_probs = True)
probs_DATA = test_classifier_data(training_data_DATA, classifier,ret_probs = True)
probs_MC = test_classifier_data(training_data_MC, classifier,ret_probs = True)
