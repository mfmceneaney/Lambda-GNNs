...
runJobs="${working_folder}/runDistort_testing.sh"
...
for value in $(seq $start_value $increment $end_value); do
  ...
  script_filename="TESTING_distort_${value_string}.sh"
#   if (( i <= 2 )); then
...
#SBATCH --mem=32G
#SBATCH --output=${output_folder}/TESTING_output_${value_string}.txt
#SBATCH --error=${error_folder}/TESTING_error_${value_string}.txt
...
/hpc/group/vossenlab/rck32/miniconda3/envs/venv/bin/python3 Distortion_NF_gauss_testing.py --Date $current_date --num_epochs $epochs --Sigma $value --extra_info "gauss_4_modes_sigma" --shift $shift
EOF
