...
def createGraph(batch):
    ...
    # print("DEBUGGING createGraph() : mc_label = ",mc_label)#DEBUGGING
    # print("DEBUGGING createGraph() : np.shape(mc_label) = ",np.shape(mc_label))#DEBUGGING
...
#     bg.edata['data'] = torch.tensor(np.moveaxis([deltas[key] for key in deltas.keys()],[0,1],[1,0]))
    ...
    if torch.any(torch.isnan(bg.ndata['data'])):
        print("torch.isnan(bg.ndata['data']) = ",torch.isnan(bg.ndata['data']))#DEBUGGING 
        for idx1, el in enumerate(bg.ndata['data']):
            for idx2, el2 in enumerate(el):
                if torch.isnan(el2): bg.ndata['data'][idx1][idx2] = 0.0
        print(bg.ndata['data'])#DEBUGGING
    graph = bg
    return graph
...
def apply(model,graph):
    ...
    prediction = model(graph) if not torch.any(torch.isnan(graph.ndata['data'])) else torch.tensor([[1.0,0.0]])
