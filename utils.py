...
def evaluate(model,device,eval_loader=None,dataset="", prefix="", split=1.0, max_events=1e20, log_dir="logs/",verbose=True, batch_size=32,
    ...
    f = plt.figure(figsize=figsize)
    plt.clf()
    plt.hist(probs_Y[:,1][torch.squeeze(torch.logical_and(test_Y==1,argmax_Y==1))].detach().numpy(), color='tab:red', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='True signal')
    plt.hist(probs_Y[:,1][torch.squeeze(torch.logical_and(test_Y==0,argmax_Y==1))].detach().numpy(), color='tab:orange', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='False signal')
    plt.hist(probs_Y[:,1][torch.squeeze(torch.logical_and(test_Y==1,argmax_Y==0))].detach().numpy(), color='tab:green', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='False background')
    plt.hist(probs_Y[:,1][torch.squeeze(torch.logical_and(test_Y==0,argmax_Y==0))].detach().numpy(), color='tab:blue', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='True background')
    plt.xlabel('output')
    plt.ylabel('counts')
    plt.yscale('log')
    plt.legend(loc='best')
...
def evaluate_on_data(model,device,dataset="", prefix="", split=1.0, log_dir="logs/",verbose=True,batch_size=32,
    ...
    f = plt.figure(figsize=(16,10))
    plt.clf()
    plt.hist(probs_Y[:,1][torch.squeeze(argmax_Y==1)].detach().numpy(), color='r', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='Signal')
    plt.hist(probs_Y[:,1][torch.squeeze(argmax_Y==0)].detach().numpy(), color='b', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=False, label='Background')
    plt.xlabel('output')
    plt.ylabel('counts')
    plt.yscale('log')
    plt.legend(loc='best')
