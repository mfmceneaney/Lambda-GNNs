...
def evaluate_on_data(model,device,dataset="", prefix="", split=1.0, log_dir="logs/",verbose=True,batch_size=32,
    ...
    f = plt.figure()
    plt.clf()
    plt.hist(probs_Y[:,1][torch.squeeze(argmax_Y==1)].detach().numpy(), color='r', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=True, label='Signal')
    plt.hist(probs_Y[:,1][torch.squeeze(argmax_Y==0)].detach().numpy(), color='b', alpha=0.5, range=low_high, bins=bins, histtype='stepfilled', density=True, label='Background')
    plt.xlabel('output')
    plt.ylabel('counts')
    plt.yscale('log')
    plt.legend(loc='best')
